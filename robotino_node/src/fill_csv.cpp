...
void CSV::createCSV(std::ofstream& myfile){
	...
	//fn << "./position/robot_pose.csv";
	myfile.open ("/home/wolfram/CATKINWS/src/robotino_default_pkgs/src/ros/robotino_node/position/position.csv");
			
	//myfile << "Position, pos_x, pos_y, yaw, time\n";
}
...
{	
	...
	/*std::cout<<"Position speichern? Y" << std::endl;
	std::cout<<"Abbrechen? N" << std::endl;*/
	int count=1;
	while(ros::ok())
	{	
