...
void GoalHandler::csvgoalCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  //std::cout << "Callback, Bool: " << received_goal << " goal: " << goal_input << std::endl;
...
void GoalHandler::ExecuteGoal(){
	//std::cout << "execute goal vielleicht?, Bool: " << received_goal << " Goal: " << goal_input << std::endl; 
    if(received_goal==true){
        //std::cout << "execute goal wirklich, Bool: " << received_goal << " Goal: " << goal_input << std::endl; 
...
void GoalHandler::read_csv(){
    ...
    //fin.open("robotino@robotino:/home/robotino/catkin_ws/src/robotino_2dnav/positions/positions.csv", std::ios::in);
    std::vector<std::string> row;
    std::string line, word, temp, oldposition;
    //std::cout << "Ist Goal leer?, Bool: " << received_goal << " Goal: " << goal_input << std::endl; 
    received_goal=false;
    if(goal_input.empty()==false){
        received_goal=true; 
        //std::cout << "Lese CSV, Bool: " << received_goal << " Goal: " << goal_input << std::endl; 
        while (std::getline(fin, line)&&ros::ok()) {
            ...
            if (oldposition==goal_input){
                ...
                /*std::cout<< "position x: " << std::stof(row[1]) << std::endl;
                std::cout<< "position y: " << std::stof(row[2]) << std::endl;
                std::cout<< "position z: " << 0 << std::endl;
                std::cout<< "orientation x: " << std::stof(row[3]) << std::endl;
                std::cout<< "orientation y: " << std::stof(row[4]) << std::endl;
                std::cout<< "orientation z: " << std::stof(row[5]) << std::endl;
                std::cout<< "orientation w: " << std::stof(row[6]) << std::endl;*/
            }
        }
        if (count==0){
